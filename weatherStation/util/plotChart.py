...
def plot_temperature_chart(inner_temperature, outer_temperature, figsize=configs.fig_size, title=None,
                           file_name='temperature.png'):
	fig, ax = plt.subplots(figsize=figsize)

	# plot the data
	p1, = ax.plot(inner_temperature[configs.TIMESTAMP], inner_temperature[configs.TEMPERATURE], '-',
	              label=INNER_TEMPERATURE, linewidth=1)
	p2, = ax.plot(outer_temperature[configs.TIMESTAMP], outer_temperature[configs.TEMPERATURE], '--',
	              label=OUTER_TEMPERATURE, linewidth=1)
	...
	plt.xticks(inner_temperature[configs.TIMESTAMP][::6], inner_temperature[configs.TIME].map(lambda x: x[:-3])[::6],
	           rotation=45)
...
def plot_humidity_chart(inner_humidity, outer_humidity, figsize=configs.fig_size, title=None, file_name='humidity.png'):
	...
	# plot the data
	p1, = ax.plot(inner_humidity[configs.TIMESTAMP], inner_humidity[configs.HUMIDITY], '-', label=INNER_HUMIDITY,
	              linewidth=1)
	p2, = ax.plot(outer_humidity[configs.TIMESTAMP], outer_humidity[configs.HUMIDITY], '--', label=OUTER_HUMIDITY,
	              linewidth=1)
	...
	# set the ticks of the x axis for each 6th value (full hour)
	plt.xticks(inner_humidity[configs.TIMESTAMP][::6], inner_humidity[configs.TIME].map(lambda x: x[:-3])[::6],
	           rotation=45)
	plt.grid(linestyle='dashed', color='lightgray')
	plt.legend()
